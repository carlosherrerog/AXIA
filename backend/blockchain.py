...
WATCH_NFT_ADDRESS = os.getenv("WATCH_NFT_ADDRESS")
MARKETPLACE_ADDRESS = os.getenv("MARKETPLACE_ADDRESS")
WATCH_AUCTION_ADDRESS = os.getenv("WATCH_AUCTION_ADDRESS") # Contrato de subastas
...
def get_full_watch_profile(token_id: int) -> dict:
    ...
    try:
        ...
        if marketplace_contract:
            try:
                approved_logs = marketplace_contract.events.AuthenticityApproved.get_logs(
                    from_block=0,
                    to_block='latest',
            ...
            try:
                rejected_logs = marketplace_contract.events.AuthenticityRejected.get_logs(
                    from_block=0,
                    to_block='latest',
                    argument_filters={'tokenId': token_id}
                )
...
def get_ownership_history_from_chain(token_id: int) -> list:
    ...
    try:
        ...
        transfer_logs = watchNFT_contract.events.Transfer.get_logs(
            from_block=0,
            to_block='latest',
            argument_filters={'tokenId': token_id}
        )
    ...
    if marketplace_contract:
        try:
            sale_logs = marketplace_contract.events.SaleCompleted.get_logs(
                from_block=0,
                to_block='latest',
                argument_filters={'tokenId': token_id}
            )
